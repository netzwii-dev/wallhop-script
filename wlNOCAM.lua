...
-- BLUE GLOW
local blueHighlight = nil
local blueLight = nil

local function setBlueGlow(state)
	if blueHighlight then
		blueHighlight.Enabled = state
	end
	if blueLight then
		blueLight.Enabled = state
	end
end

local function createBlueGlow(char)
	if blueHighlight then
		blueHighlight:Destroy()
		blueHighlight = nil
	end
	if blueLight then
		blueLight:Destroy()
	...
	blueHighlight = Instance.new("Highlight")
	blueHighlight.Name = "DoubleJumpBlueGlow"
	blueHighlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
	blueHighlight.FillColor = Color3.fromRGB(0, 170, 255)
	blueHighlight.FillTransparency = 0.45
	blueHighlight.OutlineColor = Color3.fromRGB(120, 220, 255)
	blueHighlight.OutlineTransparency = 0.05
	blueHighlight.Enabled = false
	blueHighlight.Parent = char

	blueLight = Instance.new("PointLight")
	blueLight.Name = "DoubleJumpBlueLight"
	blueLight.Color = Color3.fromRGB(0, 170, 255)
	blueLight.Brightness = 1.8
	blueLight.Range = 10
...
RunService.Heartbeat:Connect(function()
	...
	local shouldGlow = canDoubleJump and readyByCooldown and stillValid and not blockDoubleJump

	setBlueGlow(shouldGlow)
end)
...
print("HHHhhumanoid Wallhop - Loaded Successfully ✅")
